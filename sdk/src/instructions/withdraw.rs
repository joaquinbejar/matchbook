...
//!
//! Builds the instruction to withdraw tokens from a user's OpenOrders account.
...
/// Builder for the Withdraw instruction.
#[derive(Debug, Clone)]
...
impl WithdrawBuilder {
    ...
    /// Sets the base amount to withdraw.
    #[must_use]
    ...
    /// Sets the quote amount to withdraw.
    #[must_use]
    ...
    pub fn build(self) -> Result<Instruction, SdkError> {
        ...
        let mut data = vec![3u8; 8]; // Placeholder discriminator for Withdraw
...
mod tests {
    ...
    #[test]
    fn test_withdraw_builder_new() {
        let program_id = test_program_id();
        let builder = WithdrawBuilder::new(program_id);
        assert_eq!(builder.program_id, program_id);
    }

    #[test]
    fn test_withdraw_builder_chain() {
        let program_id = test_program_id();
    ...
    #[test]
    fn test_withdraw_builder_build() {
        let program_id = test_program_id();
        let owner = Pubkey::new_unique();
        let market = Pubkey::new_unique();
        let user_base = Pubkey::new_unique();
        ...
        let ix = WithdrawBuilder::new(program_id)
            .owner(owner)
            .market(market)
            .user_base_account(user_base)
            .user_quote_account(user_quote)
    ...
    #[test]
    fn test_withdraw_builder_build_missing_market() {
        let program_id = test_program_id();
        let owner = Pubkey::new_unique();
        let user_base = Pubkey::new_unique();
        let user_quote = Pubkey::new_unique();

        let result = WithdrawBuilder::new(program_id)
            .owner(owner)
            .user_base_account(user_base)
            .user_quote_account(user_quote)
            .build();
